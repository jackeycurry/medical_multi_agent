...
├── middleware/
│   ├── auth.py        # JWT 解码, get_current_user, get_optional_user, require_role, require_hospital
│   └── api_key.py     # X-API-Key 鉴权 + 滑动窗口限流
└── schemas/           # Pydantic 请求/响应模型
```

**认证流程**: 所有 API 通过 `Authorization: Bearer <JWT>` 认证。JWT payload: `{sub: user_id, role: str, hid: hospital_id}`。`require_role(*roles)` 工厂函数生成角色守卫依赖。`get_optional_user()` 用于需要可选认证的端点（token 无效时返回 None 而非抛异常）。
...
├── components/        # 共享组件: AppLayout, DoctorCard, DoctorForm, ChatBubble, ChatInput, StatCard
...
SQLite 单文件 (`backend/data/ai_doctor.db`)，通过 `check_same_thread=False` 支持 FastAPI 异步。种子用户密码均为 `123456`，admin 密码为 `admin123`。AI 医生种子数据在 `backend/data/seed_doctors.json`。`main.py` 启动时自动调用 `seed_all()`，仅在表为空时插入（幂等）。

**关键约束**: `python run.py` 必须以 `backend/` 为当前工作目录，因为 `DATABASE_URL` 默认使用相对路径 `sqlite:///./data/ai_doctor.db`。

### hospital_admin 跨院视图

`/api/hospital/*` 的守卫同时允许 `hospital_admin` 和 `super_admin` 访问。`super_admin` 发请求时 `hospital_id` 为 None，后端将其视为全院视图（返回所有医院数据），因此 super_admin 可通过 hospital 端点统管所有医院。

### CORS 与中间件

`main.py` 中 `app.add_middleware()` 按**注册顺序的逆序**执行（FastAPI 惯例）：CORS 中间件后注册，先执行。`allow_origins=["*"]` 允许任意来源跨域。
